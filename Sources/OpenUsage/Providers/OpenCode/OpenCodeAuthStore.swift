...
struct OpenCodeAuthStore: Sendable {
    ...
    /// a future non-object field like a schema marker) so one odd value can't hide a valid key. A
    /// present-but-unreadable file is treated as "no key" (the scanner's usage probe is the other signal).
    func goAPIKey() -> String? {
        guard let text = (try? files.readTextIfPresent(authFilePath)) ?? nil,
